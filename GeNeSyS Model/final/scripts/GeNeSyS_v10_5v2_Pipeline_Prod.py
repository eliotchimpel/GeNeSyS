...
# -*- coding: utf-8 -*-
"""
GeNeSyS_v10_5v2_pipeline_Prod.py

UNIFIED PIPELINE FOR GENESYS V10.5v2 COSMOLOGICAL MODEL (BACKGROUND-ONLY)
=====================================================================
...
def init_logging(output_dir: str | None) -> None:
    """Initialize a simple tee log in the output directory for traceability."""
    ...
    try:
        ...
        _LOG_PATH = str(Path(output_dir) / '_GeNeSyS_v10_5v2_pipeline.log')
        _LOG_FH = open(_LOG_PATH, 'a', encoding='utf-8')
        ts = datetime.utcnow().isoformat()
        _LOG_FH.write(f"\n=== GeNeSyS PIPELINE RUN {ts} UTC ===\n")
        _LOG_FH.flush()
    except Exception:
        _LOG_FH = None
        _LOG_PATH = None
...
def info(msg: str) -> None:
    """Print info message (and tee to log if enabled)."""
    msg2 = f"[INFO] {msg}"
    print(msg2)
    global _LOG_FH
    if _LOG_FH is not None:
...
def warn(msg: str) -> None:
    """Print warning message (stderr) and tee to log if enabled."""
    msg2 = f"[WARN] {msg}"
...
def apply_params_json(args: argparse.Namespace) -> None:
    ...
    for k, v in payload.items():
        ...
        if k in ("fixed_parameters", "data_files", "cmb_theta_gate", "theta_gate", "eb_contract"):
            continue
...
"tau_fast": args.tau_fast,
    ...
    info(df_sorted[cols].head(10).to_string(index=False))
...
xmin=args.xmin,
    ...
    out_path = Path(args.output)
    out_path.parent.mkdir(parents=True, exist_ok=True)
    df_out = pd.DataFrame({"z": z, "E": E, "E_genesys": E})
...
def cmd_confront(args: argparse.Namespace) -> None:
    ...
    info(json.dumps(out, indent=2))
def cmd_all(args: argparse.Namespace) -> None:
    """Run scan → best-ez → confront in one command."""
    apply_params_json(args)
    apply_output_dir(args)
    init_logging(getattr(args, 'output_dir', None))
    # 1. Scan
    ...
    # 4. Summary
    try:
        summarize_run(Path(args.scan_out).with_suffix('.meta.json'), Path(args.params_out), Path(args.verdict_out))
    except Exception as _e:
